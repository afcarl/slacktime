"""Slightly modified version of the Optunity hyperparameter optimization library.

Not a complete implementation, so requires Optunity to be installed.
(Optunity is available at https://github.com/claesenm/optunity
 Copyright (c) 2014 KU Leuven, ESAT-STADIUS. All rights reserved.)

Current modifications:
Added solver_name parameter to maximize_structured and minimize_structured so you can specify a specific solver,
instead of being forced to use particle swarm. (Particle swarm is still the default though.)
"""
...
from optunity.util import DocumentedNamedTuple as DocTup
from optunity.constraints import wrap_constraints
import optunity.parallel as par
...
def maximize_structured(f, search_space, solver_name='particle_swarm', num_evals=50, pmap=map):
    """Basic function maximization routine. Maximizes ``f`` within
    the given box constraints.
    :param f: the function to be maximized
    ...
    suggestion = suggest_solver(num_evals, solver_name, **box)
    solver = make_solver(**suggestion)
    solution, details = optimize(solver, f, maximize=True, max_evals=num_evals,
...
def minimize_structured(f, search_space, solver_name='particle_swarm', num_evals=50, pmap=map):
    """Basic function minimization routine. Minimizes ``f`` within
    the given box constraints.
